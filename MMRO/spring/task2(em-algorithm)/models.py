...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        posteriors = []
        for sentence in parallel_corpus:
            s, t = sentence.source_tokens, sentence.target_tokens
            probs = self.translation_probs[s][:, t]
            probs /= (probs.sum(axis=0, keepdims=True) + 1e-9)
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for sentence, posterior in zip(parallel_corpus, posteriors):
            s, t = sentence.source_tokens, sentence.target_tokens
            probs = self.translation_probs[s][:, t]
            norm_factor = len(s) * posterior + 1e-9
            elbo += np.sum(posterior * (np.log(probs + 1e-9) - np.log(norm_factor)))
        return elbo
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for sentence, posterior in zip(parallel_corpus, posteriors):
            s, t = sentence.source_tokens, sentence.target_tokens
            np.add.at(self.translation_probs, (s[:, None], t[None, :]), posterior)
        self.translation_probs /= (self.translation_probs.sum(axis=1, keepdims=True) + 1e-9)
    ...
    def align(self, sentences):
        ...
        for sentence in sentences:
            s, t = sentence.source_tokens, sentence.target_tokens
            probs = self.translation_probs[s][:, t]
            best_alignments = probs.argmax(axis=0) + 1
            alignments.append(list(zip(best_alignments, range(1, len(t) + 1))))
        return alignments
...
class WordPositionAligner(WordAligner):
    ...
    def _e_step(self, parallel_corpus):
        ...
        for sentence in parallel_corpus:
            s, t = sentence.source_tokens, sentence.target_tokens
            src_len, tgt_len = len(s), len(t)
            joint_probs = self.translation_probs[s][:, t] * self._get_probs_for_lengths(src_len, tgt_len)
            posteriors.append(joint_probs / (joint_probs.sum(axis=0, keepdims=True) + 1e-12))
        return posteriors
    ...
    def _compute_elbo(self, parallel_corpus, posteriors):
        ...
        for sentence, posterior in zip(parallel_corpus, posteriors):
            s, t = sentence.source_tokens, sentence.target_tokens
            src_len, tgt_len = len(s), len(t)
            trans_probs = self.translation_probs[s][:, t]
            align_probs = self._get_probs_for_lengths(src_len, tgt_len)
            elbo += np.sum(posterior * np.log(trans_probs * align_probs + 1e-12))
            elbo -= np.sum(posterior * np.log(posterior + 1e-12))
        return elbo

    def _m_step(self, parallel_corpus, posteriors):
        self.translation_probs.fill(0.0)
        length_counts = defaultdict(lambda: None)
        for sentence, posterior in zip(parallel_corpus, posteriors):
            s, t = sentence.source_tokens, sentence.target_tokens
            src_len, tgt_len = len(s), len(t)
            np.add.at(self.translation_probs, (s[:, None], t[None, :]), posterior)
            if length_counts[(src_len, tgt_len)] is None:
                length_counts[(src_len, tgt_len)] = np.zeros((src_len, tgt_len), dtype=np.float32)
            length_counts[(src_len, tgt_len)] += posterior
        self.translation_probs /= (self.translation_probs.sum(axis=1, keepdims=True) + 1e-12)
        for (src_len, tgt_len), counts in length_counts.items():
            if counts is not None:
                self.alignment_probs[(src_len, tgt_len)] = counts / (counts.sum(axis=0, keepdims=True) + 1e-12)
        return self._compute_elbo(parallel_corpus, posteriors)
